    ...
    <el-table v-loading="listLoading" :data="deviceList">
      ...
      <el-table-column label="设备图片">
        ...
        <el-table-column label="生产厂商">
            <template slot-scope="scope" v-if="scope.row.maker">
                {{ scope.row.maker }}
            </template>
        </el-table-column>

        <el-table-column label="生产价格">
      ...
      <el-table-column label="操作" width="360px">
        <template slot-scope="scope">
          <el-row :gutter="30">
            ...
            <el-col :span="6">
              <el-button size="mini" @click="showRepairDialog(scope.row)">维修设备</el-button>
            </el-col>
            <el-col :span="6">
              <el-button size="mini" @click="showCostDialog(scope.row)">购买设备</el-button>
            </el-col>
    ...
    <el-dialog :title="dialogTitleMap[dialogStatus]" :visible.sync="formDialogVisible" width="45%">
      <el-form :model="template" label-position="left">
        ...
        <el-form-item label="设备名称" label-width="90px">
          ...
          <el-form-item label="生产厂商" label-width="90px">
              <el-input v-model="template.maker" />
          </el-form-item>

          <el-form-item label="生产金额" label-width="90px">
              <el-input v-model="template.price" />
          </el-form-item>
...
<!--    申请借用设备 Dialog-->
    <el-dialog title="填写申请单" :visible.sync="applyDialogVisible">
      ...
      <!--    维修设备 Dialog-->
    <el-dialog title="填写维修单" :visible.sync="repairDialogVisible">
      <el-form :model="applyForm" label-position="left">
        <el-form-item label="维修时间">
          <el-date-picker v-model="applyForm.promiseTimestamp" type="datetime" value-format="timestamp" />
        </el-form-item>
        <el-form-item label="维修理由">
          <el-input v-model="applyForm.borrowReason" :autosize="{ minRows: 4, maxRows: 8}" type="textarea" />
        </el-form-item>
      </el-form>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="createrepair()">提交维修申请</el-button>
      </div>
    </el-dialog>

<!--    购买设备 Dialog-->
    <el-dialog title="填写购买单据" :visible.sync="costDialogVisible">
      <el-form :model="applyForm" label-position="left">
        <el-form-item label="预计购买时间">
          <el-date-picker v-model="applyForm.promiseTimestamp" type="datetime" value-format="timestamp" />
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="createcost()">提交购买申请</el-button>
      </div>
    </el-dialog>
...
<script>
import {getDeviceList, countDeviceList, addDevice, updateDevice, deleteDevice, applyDevice,repairDevice,costDevice} from "@/api/device";
...
export default {
  ...
  methods: {
    ...
    createrepair() {
      ...
      api.repairDevice(this.applyForm).then(() => {
    ...
    createcost() {
      ...
      api.costDevice(this.applyForm).then(() => {
