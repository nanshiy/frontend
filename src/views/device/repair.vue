<template>
    <div class="app-container">
        ...
        <el-table v-loading="listLoading" :data="deviceApplyList">
            ...
            <el-table-column label="维修时间" width="160px">
                <template slot-scope="scope">
            ...
            <el-table-column label="修理厂家">
                <template slot-scope="scope">
                    {{ scope.row.plant }}
                </template>
            </el-table-column>

            <el-table-column label="修理费用">
                <template slot-scope="scope">
                    {{ scope.row.cost }}
                </template>
            </el-table-column>
        ...
        <el-dialog :visible.sync="progressDialogVisible" title="报修进度" width="55%">
            <el-steps :active="processInfo.step" align-center>
                ...
                <el-step title="正在报修" :description="processInfo.grantTimestamp | parseTime('{y}-{m}-{d} {h}:{i}:{s}')"></el-step>
                <el-step title="报修结束" :description="processInfo.returnTimestamp | parseTime('{y}-{m}-{d} {h}:{i}:{s}')"></el-step>
                <el-step title="确认完成" :description="processInfo.finishTimestamp | parseTime('{y}-{m}-{d} {h}:{i}:{s}')"></el-step>
            </el-steps>
        </el-dialog>
...
<script>
import { getApplyList, countApplyList, updateApply, addRecord } from "@/api/repair";
import {mapGetters} from "vuex";
import {countLaboratoryList, getLaboratoryList, getLaboratoryListByUserId} from "../../api/laboratory";
